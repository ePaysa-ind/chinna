...
class AuthActivity : AppCompatActivity() {
    ...
    private fun setupViews() {
        ...
        // Setup real-time validation for name and village
        ...
        binding.etVillage.addTextChangedListener(object : android.text.TextWatcher {
            override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {}
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                val input = s.toString()
                if (!input.matches(namePattern) && input.isNotEmpty()) {
                    binding.etVillage.setText(input.replace(Regex("[^a-zA-Z\\s]"), ""))
                    binding.etVillage.setSelection(binding.etVillage.text?.length ?: 0)
                    showError("Village cannot contain numbers or special characters")
                }
            }
            override fun afterTextChanged(s: android.text.Editable?) {}
        })
    ...
    private fun validateInputs(): Boolean {
        ...
        // Name validation pattern - no numbers allowed
        val namePattern = "^[a-zA-Z\\s]+$".toRegex()
        ...
        return when {
            ...
            village.length < 3 -> {
                showError("Village must be at least 3 characters")
                false
            }
            !village.matches(namePattern) -> {
                showError("Village should not contain numbers")
                false
            }
